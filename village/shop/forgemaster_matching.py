...
class ForgemasterGradientMatching(_Forgemaster):
    ...
    def _passenger_loss(self, poison_grad, target_grad, target_gnorm):
        ...
        if self.args.loss in SIM_TYPE:
            if self.args.independent_brewing:
                passenger_loss = 1 + passenger_loss / poison_norm.sqrt().detach()
            else:
...
class ForgemasterGradientMatchingNoisy(ForgemasterGradientMatching):
    """Brew passenger poison with given arguments.
